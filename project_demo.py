...
if __name__ == "__main__":
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            str_gesture = get_str_guester(up_fingers, list_lms)

            cv2.putText(img, ' %s' % (str_gesture), (90, 90), cv2.FONT_HERSHEY_SIMPLEX, 1.25, (255, 255, 0), 4,
                        cv2.LINE_AA)
            # send_result2pi(str_gesture)
            for i in ll:
                pos_x = hand.landmark[i].x * image_width
                pos_y = hand.landmark[i].y * image_height
                # 画点
